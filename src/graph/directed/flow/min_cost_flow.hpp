...
class min_cost_flow : public flow_graph<Cap, Cost> {
    ...
    if constexpr (Density_tag) {  // O(V^2)
      ...
      for (size_type src{}; src != size(); ++src) {
        if (static_cast<Cap>(0) < b[src]) {
          used[src] = true;
          newp[src] = 0;
          for (edge_impl &e : base::graph[src]) {
            if (static_cast<Cap>(0) < b[e.dst]) continue;
            if (e.avbl() && e.cost < newp[e.dst]) {
              newp[e.dst] = e.cost;
              last[e.dst] = &e;
            }
          }
      ...
      for (;;) {
        ...
        used[src] = true;
        for (edge_impl &e : base::graph[src]) {
          if (e.avbl() && newp[src] + e.cost < newp[e.dst]) {
        ...
        if (b[src] > static_cast<Cap>(0)) {
          ...
          for (edge_impl &e : base::graph[src])
            if (!(static_cast<Cap>(0) < b[e.dst]) &&
              ...
              que.emplace(e.dst, (newp[e.dst] = e.cost) - p[e.dst]);
              last[e.dst] = &e;
            }
        }
      ...
      while (!que.empty()) {
        ...
        for (edge_impl &e : base::graph[src])
          if (static_cast<Cap>(0) < e.cap && newp[e.dst] > newp[src] + e.cost) {
            que.emplace(e.dst, (newp[e.dst] = newp[src] + e.cost) - p[e.dst]);
            last[e.dst] = &e;
          }
      }
    }
 ...
 public:
  ...
  // infinity capatity
  // edge_impl *add_edge(size_type src, size_type dst, const Cost &cost);
