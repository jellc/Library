...
namespace math
{
    namespace binomial
    {
        namespace internal
        {
            ...
            inline int fact_helper(int x) { assert(x < (int)N); return x < 0 ? 0 : fact_impl_inst._fact[x]; }
            inline int invfact_helper(int x) { assert(x < (int)N); return x < 0 ? 0 : fact_impl_inst._invfact[x]; }
            inline int inv_helper(int x) { assert(x < (int)N); return x < 0 ? 0 : fact_impl_inst._inv[x]; }
        } // namespace internal
        class modint
        {
            static constexpr int mod = internal::mod;
            int dat;
          public:
            ...
            friend modint inverse(const modint &other);
            ...
            friend modint inverse(const modint &other)
            {
                ...
                int a = other.dat, b = mod, u = 1, v = 0, t;
        ...
        inline modint fact(int x) { return internal::fact_helper(x); }
        inline modint invfact(int x) { return internal::invfact_helper(x); }
        inline modint binom(int x, int y) { return fact(x) * invfact(y) * invfact(x - y); }
        inline modint fallfact(int x, int y) { return fact(x) * invfact(x - y); }
        inline modint risefact(int x, int y) { return fallfact(x + y - 1, y); }
    } // namespace binomial
